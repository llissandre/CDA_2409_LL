...
SELECT PILNOM, v.AV, v.PIL, count(v.AV)
FROM PILOTE p
JOIN VOL v ON p.PIL = v.PIL
JOIN AVION a ON v.AV = a.AV
ORDER BY PILNOM, v.AV;
-- HAVING AV = 105 AND AV = 109; 

SELECT v.PIL, count(DISTINCT AV) FROM VOL v GROUP BY PIL, AV HAVING AV = 105;
SELECT PIL, count(DISTINCT AV) FROM VOL GROUP BY PIL, AV HAVING AV = 109;