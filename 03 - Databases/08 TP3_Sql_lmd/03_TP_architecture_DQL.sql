...
SELECT client_ref, client_nom, type_client_libelle FROM clients NATURAL JOIN type_clients WHERE type_client_libelle = 'Particulier';
...
SELECT pa.projet_ref, projet_date_depot, projet_date_fin_prevue, projet_superficie_totale, projet_superficie_batie, projet_prix, count(pa.emp_matricule)
FROM projets p
JOIN participer pa ON pa.projet_ref = p.projet_ref
GROUP BY pa.projet_ref
HAVING 'Architecte' NOT IN (SELECT DISTINCT fonction_nom FROM fonctions f NATURAL JOIN employes e NATURAL JOIN participer p)
ORDER BY pa.projet_ref DESC;
...
SELECT type_travaux_id, type_travaux_libelle, max(projet_superficie_totale) AS MAXT,
max(projet_superficie_batie) AS MAXB
FROM type_travaux t
NATURAL JOIN projets
GROUP BY type_travaux_id, projet_superficie_totale WITH ROLLUP
ORDER BY type_travaux_id;

WITH 
MAXT AS (
SELECT type_travaux_id, type_travaux_libelle, projet_ref, max(projet_superficie_totale) AS 'MAX_SUP_T'
FROM type_travaux t
NATURAL JOIN projets
GROUP BY type_travaux_id, projet_ref),
MAXB AS (
SELECT type_travaux_id, type_travaux_libelle, projet_ref, max(projet_superficie_batie) AS 'MAX_SUP_B'
FROM type_travaux t
NATURAL JOIN projets
GROUP BY type_travaux_id, projet_ref)

SELECT MAXT.type_travaux_id, MAXT.type_travaux_libelle, MAXT.projet_ref, MAX_SUP_T,  MAXB.projet_ref, MAX_SUP_B FROM MAXT JOIN MAXB
ON MAXT.type_travaux_id = MAXB.type_travaux_id
ORDER BY MAXT.type_travaux_id;
...
client_nom AS 'Nom du client', client_telephone AS 'Téléphone du client', concat(a.adresse_num_voie, ' ', a.adresse_voie, '  ',
a.adresse_code_postal, ' ', a.adresse_ville), type_travaux_libelle AS 'Type de travaux', type_projet_libelle AS 'Type de projet'
FROM projets p
...
SELECT projet_date_depot AS 'date de dépôt', projet_date_fin_prevue AS 'Date de fin prévue', projet_date_fin_effective AS 'Date de fin effective', projet_prix AS 'Prix du projet', 
...
SELECT projet_ref, client_nom AS 'Nom du client'
FROM projets p
JOIN clients c ON c.client_ref = p.client_ref
JOIN adresses a ON a.adresse_id = c.adresse_id
