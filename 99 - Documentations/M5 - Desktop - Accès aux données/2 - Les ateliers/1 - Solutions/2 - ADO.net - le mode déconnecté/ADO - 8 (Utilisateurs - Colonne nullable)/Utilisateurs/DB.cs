    ...
    internal class DB
    {
        ...
        public int InsertUtilisateur(string nom, string prenom, DateTime dtNaiss)
        {
            try
            {
                ...
                var result = _dbConnection.Query<int>(q, new { nom, prenom, dtNaiss });
                return result.Single();
            }
            finally { _dbConnection.Close(); }
        }

        public int InsertIfNotExistUtilisateur(string nom, string prenom, DateTime? dtNaiss)
        {
            try
            {
                _dbConnection.Open();
                var q = "INSERT IGNORE INTO Utilisateurs (Nom,Prenom,DtNaiss) VALUES (@nom, @prenom, @dtNaiss); SELECT LAST_INSERT_ID()";
                var result = _dbConnection.Query<int>(q, new { nom, prenom, dtNaiss });
                return result.Single();
            }
            finally { _dbConnection.Close(); }
